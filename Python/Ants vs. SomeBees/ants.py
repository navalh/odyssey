...
# Name: Naval Handa, Shamoil Gulamhusein
# Email: nhanda@berkeley.edu, shamoil@berkeley.edu
...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        elif type(insect) is QueenAnt and insect.original_queen:
            return
        else:
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != None:
            if current_place == hive:
                return None
            if current_place.bees != [] and transitions >= self.min_range and transitions <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            game_print('{0} ran out of armor and expired'.format(self))
            fixed_length_bees = len(self.place.bees)
            i = 0
            while i < fixed_length_bees:
                if self.place.bees[i].armor > 3:
                    self.place.bees[i].reduce_armor(self.damage)
                    i = i + 1
                else:
                    self.place.bees[i].reduce_armor(self.damage)
                    fixed_length_bees = fixed_length_bees - 1
            self.place.remove_insect(self)
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        ...
        self.residing_place = ant_queen

    @property
    def bees(self):
        return self.original_place.bees + self.residing_place.bees
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        self.original_queen = False
        QueenAnt.number_of_queens = QueenAnt.number_of_queens + 1
        if QueenAnt.number_of_queens == 1:
            self.original_queen = True
    ...
    def action(self, colony):
        ...
        if QueenAnt.number_of_queens > 1 and not(self.original_queen): 
            ...
            current_place = self.place
            if current_place.ant.container and current_place.ant not in self.double_damage_ants:
                current_place.ant.damage = 2 * current_place.ant.damage
                self.double_damage_ants.append(current_place.ant)
            colony.queen = QueenPlace(colony.queen, current_place)
            next_place = self.place.entrance
            previous_place = self.place.exit

            while next_place.entrance != None or previous_place.exit != None: 
                if next_place.ant != None and next_place.ant not in self.double_damage_ants:
                    next_place.ant.damage = 2 * next_place.ant.damage
                    self.double_damage_ants.append(next_place.ant)
                    if next_place.ant.container and next_place.ant.ant != None:
                        next_place.ant.ant.damage = 2 * next_place.ant.ant.damage
                        self.double_damage_ants.append(next_place.ant.ant)

                if previous_place.ant != None and previous_place.ant not in self.double_damage_ants:
                    previous_place.ant.damage = 2 * previous_place.ant.damage
                    self.double_damage_ants.append(previous_place.ant)
                    if previous_place.ant.container and previous_place.ant.ant != None:
                        previous_place.ant.ant.damage = 2 * previous_place.ant.ant.damage
                        self.double_damage_ants.append(previous_place.ant.ant)

                if next_place.entrance != None:
                    next_place = next_place.entrance
                if previous_place.exit != None:
