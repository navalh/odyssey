...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Not the correct number of arguments given.")
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        for i in range(len(args)):
            python_argument_list.append(args[i])
        if self.use_env:
            python_argument_list.append(env)
        try:
            return self.fn(*python_argument_list), None
        except TypeError:
            raise SchemeError
...
class LambdaProcedure(Procedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            this_frame = Frame.make_call_frame(self.env, self.formals, args)
            return self.body, this_frame
        else:
            this_frame = Frame.make_call_frame(self.env, self.formals, args)
            return scheme_eval(self.body, this_frame), None
...
class MuProcedure(LambdaProcedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            # Implemented in Question 22.
            this_frame = Frame.make_call_frame(env, self.formals, args) 
            return self.body, this_frame
        else:
            this_frame = Frame.make_call_frame(env, self.formals, args) 
            return scheme_eval(self.body, this_frame), None
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        return vals[0], None
    elif scheme_pairp(target) and scheme_symbolp(target[0]):
        vals_for_lambda = Pair(target.second, vals.second)
        env.bindings[vals[0][0]] = do_lambda_form(vals_for_lambda, env)[0]
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    predicate = scheme_eval(vals[0], env).__bool__()
    if predicate:
        return vals[1], env
    elif len(vals) == 2:
        return okay, None
...
def check_formals(formals):
    ...
    """
    if formals is nil or formals._list_end() is nil:
        set_of_formals = set()
        for i in range(len(formals)):
            if not scheme_symbolp(formals[i]) or formals[i] in set_of_formals:
                raise SchemeError ("Not a list of only (unique) symbols.")
            set_of_formals.add(formals[i])
    else:
        raise SchemeError ("Not a well-formed list, i.e., has a dot in it.")
