...
def shuffle(cards):
    ...
    shuffled_cards = [0]*len(cards)
    for i in range(len(cards)//2):
        shuffled_cards[2*i] = cards[i]
        shuffled_cards[2*i + 1] = cards[len(cards)//2 + i]
...
def reaches_circularity(G, v0):
    ...
    False
    """
    def is_path_to_cycle(v1, vertices_left):
        if vertices_left <= 0:
            return False
        else:
            for w in G[v1]:
                if v0 == w:
                    return True
                if is_path_to_cycle(w, vertices_left - 1):
                    return True
            return False
    return is_path_to_cycle(v0, len(G))
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, attempted_password):
        nonlocal balance, attempts_left, attempts
        if attempted_password == password and attempts_left > 0:
            if amount > balance:
               return 'Insufficient funds'
            balance = balance - amount
            return balance
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    validate = withdraw(0, old_password)
    if not(type(validate) == str):
        def new_withdraw(amount, attempted_password):
            if attempted_password == old_password or attempted_password == new_password:
